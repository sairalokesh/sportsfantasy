...
@Repository
public interface GameParticipantPointsRepository
    extends CrudRepository<GameParticipantPoints, Long> {
  ...
  @Query("select p from GameParticipantPoints as p where p.gameQuestions.id = ?1 and p.gameParticipants.pid =?2")
  GameParticipantPoints getSelectedApiPArticipantPoints(Long questionId, Long pid);
